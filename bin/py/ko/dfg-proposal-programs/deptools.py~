	...
	def _buildQuery(self,tuples):
		if tuples==():
			...
			for key, val in tuples:
				if val.startswith('!'):
					retval= retval + " and dn.getAttribute('%s') != '%s'"%(key,val[1:])
				elif val.find('or') > 0:
					left, right = val.split(' or ')
					retval= retval + " and (dn.getAttribute('%s') == '%s' or dn.getAttribute('%s') == '%s')"%(key,left,key,right)
				else:
					retval= retval + " and dn.getAttribute('%s') == '%s'"%(key,val)
